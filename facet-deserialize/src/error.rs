...
//use crate::{Cooked, Format, Outcome, Span, cook_span_dispatch};
...
impl<'input, C, 'shape> DeserError<'input, C, 'shape> {
    ...
    // /// Converts this error's span to a Cooked span
    // pub fn with_cooked_span<F>(
    //     self,
    //     format: &F,
    //     input: &'input F::Input<'input>,
    // ) -> DeserError<'input, Cooked>
    // where
    //     F: Format,
    // {
    //     let cooked_span = cook_span_dispatch!(format, self.span, input);
    //     DeserError {
    //         input: self.input,
    //         span: cooked_span,
    //         kind: self.kind,
